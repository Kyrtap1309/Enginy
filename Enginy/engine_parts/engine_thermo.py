...
def mach_solver(
    ...
    while not converged and n_iter <= max_iterations:
        ...
        T_static_out = gas_in.T + (
            V_in**2 / (2 * gas_in.cp) - V_out_guess**2 / (2 * gas_out.cp)
        )
        p_total_out = gas_in.P * (1 + eta * V_in**2 / (2 * gas_in.cp * gas_in.T)) ** (
            gamma_in / (gamma_in - 1)
        )
        p_static_out = p_total_out * (T_total_out / T_static_out) ** (
...
def compressor_solver(
    ...
    )  # internal object to keep track of gas properties
    stage_gas.X = gas_management.comp_air

    # compressor output data
    compressor_work = (
        0  # collector for specific work used to compress gas, for all stages, in kJ/kg
    )
    ...
    while not converged and n_iter <= max_iterations:
        ...
        for st_counter in range(n_stages):
            ...
            p_total = (
                get_p_total(stage_gas.P, gamma, M_in)
                * compress_stage
            ...
            T_total = (
                T_total_in
                / comp_eta
                * ((p_total / p_total_in) ** ((gamma - 1) / gamma) - 1)
                + T_total_in
            )
        ...
        if n_stages > 2:  # typical multi-stage case
            ...
            max_delta_t = max(
                stages_T_out[1] - stages_T_out[0], stages_T_out[0] - gas_in.T
            )
